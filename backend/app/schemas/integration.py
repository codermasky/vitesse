...
class APIEndpoint(BaseModel):
    ...
    rate_limit: Optional[str] = Field(default=None, description="Rate limit info if available")


class APISpecification(BaseModel):
    """API specification discovered by Ingestor."""
    source_url: str = Field(..., description="Original API documentation URL or Swagger spec URL")
    ...
    auth_config: Dict[str, Any] = Field(default_factory=dict, description="Auth configuration details")
    endpoints: List[APIEndpoint] = Field(default_factory=list, description="Discovered endpoints")
    ...
    pagination_style: Optional[str] = Field(default=None, description="Pagination style (offset, cursor, etc)")
...
class DataTransformation(BaseModel):
    ...
    transform_type: str = Field(default="direct", description="Transform type (direct, mapping, function, etc)")
    transform_config: Dict[str, Any] = Field(default_factory=dict, description="Transform-specific config")
    required: bool = Field(default=False, description="Is this field required")
    default_value: Optional[Any] = Field(default=None, description="Default value if source is empty")
...
class MappingLogic(BaseModel):
    ...
    transformations: List[DataTransformation] = Field(default_factory=list, description="Transformation rules")
    pre_sync_hook: Optional[str] = Field(default=None, description="Pre-sync custom logic")
    post_sync_hook: Optional[str] = Field(default=None, description="Post-sync custom logic")
...
class HealthScore(BaseModel):
    ...
    endpoint_coverage: float = Field(..., ge=0, le=100, description="% of endpoints tested")
    ...
    test_results: List[TestResult] = Field(default_factory=list, description="Detailed test results")
...
class DeploymentConfig(BaseModel):
    ...
    region: Optional[str] = Field(default="us-east-1", description="AWS region if cloud")
    instance_type: Optional[str] = Field(default="t3.small", description="Instance type")
    ...
    secrets: Dict[str, str] = Field(default_factory=dict, description="Secrets (stored separately)")
    ...
    max_replicas: int = Field(default=10, description="Maximum replicas for autoscaling")
...
class IntegrationInstance(BaseModel):
    ...
    source_api_spec: APISpecification = Field(..., description="Source API specification")
    dest_api_spec: APISpecification = Field(..., description="Destination API specification")
    mapping_logic: Optional[MappingLogic] = Field(default=None, description="Applied mapping logic")
    health_score: Optional[HealthScore] = Field(default=None, description="Latest health assessment")
    deployment_config: DeploymentConfig = Field(..., description="Deployment configuration")
    container_id: Optional[str] = Field(default=None, description="Container/Pod ID if deployed")
