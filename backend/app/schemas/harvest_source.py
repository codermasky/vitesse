...
class HarvestSourceBase(BaseModel):
    ...
    name: str = Field(..., min_length=1, max_length=255, description="Display name for the harvest source")
    type: str = Field(..., description="Type of harvest source (api_directory, marketplace, github, documentation)")
    ...
    enabled: bool = Field(True, description="Whether this source is enabled for harvesting")
    priority: int = Field(0, description="Priority order for harvesting (higher = first)")
    auth_type: Optional[str] = Field(None, description="Authentication type (none, api_key, oauth2, basic)")
    auth_config: Optional[Dict[str, Any]] = Field(None, description="Authentication configuration")
    category: Optional[str] = Field(None, description="API category this source specializes in")
    tags: Optional[List[str]] = Field(None, description="Tags for filtering and organization")
