...
class AgentCommunicationLog(BaseModel):
    ...
    message_type: str = Field(
        ..., description="Type of communication (task_assignment, task_update, etc.)"
    )
...
class AgentMetrics(BaseModel):
    ...
    uptime_percentage: float = Field(
        ..., ge=0, le=100, description="Agent uptime percentage"
    )
    tasks_completed_today: int = Field(..., ge=0, description="Tasks completed today")
    tasks_completed_week: int = Field(
        ..., ge=0, description="Tasks completed this week"
    )
    average_task_duration: int = Field(
        ..., ge=0, description="Average task duration in seconds"
    )
    success_rate: float = Field(
    ...
    collaboration_score: int = Field(
        ..., ge=0, le=100, description="Collaboration effectiveness score"
    )
    response_time_p95: int = Field(
        ..., ge=0, description="95th percentile response time in seconds"
    )
...
class CollaborationStats(BaseModel):
    ...
    active_workflows: int = Field(..., ge=0, description="Number of active workflows")
    total_communications_today: int = Field(
        ..., ge=0, description="Total communications today"
    )
    average_collaboration_score: float = Field(
        ..., ge=0, le=100, description="Average collaboration score"
    )
    system_uptime: float = Field(
        ..., ge=0, le=100, description="System uptime percentage"
    )
    ...
    error_rate: float = Field(
        ..., ge=0, le=100, description="System error rate percentage"
    )
    peak_collaboration_hour: str = Field(
        ..., description="Peak collaboration hour (HH:MM)"
    )
