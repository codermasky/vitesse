...
class HarvestJobCreate(BaseModel):
    ...
    harvest_type: str = Field(..., description="Type of harvest to perform (full, financial, api_directory, etc.)")
    source_ids: Optional[List[int]] = Field(None, description="Specific source IDs to harvest from (optional)")

    @validator("harvest_type")
    def validate_harvest_type(cls, v):
        valid_types = ["full", "financial", "api_directory", "documentation", "code_repositories"]
...
class HarvestJobResponse(BaseModel):
    ...
    total_sources: int = Field(..., ge=0, description="Total number of sources to process")
    processed_sources: int = Field(..., ge=0, description="Number of sources processed")
    successful_harvests: int = Field(..., ge=0, description="Number of successful harvests")
...
class HarvestJobStats(BaseModel):
    ...
    average_job_duration: int = Field(..., ge=0, description="Average job duration in seconds")
    total_apis_harvested: int = Field(..., ge=0, description="Total APIs harvested across all jobs")
